...
@section('content')
    <main class="min-h-screen bg-white flex items-start justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div class="w-full max-w-md space-y-8">
            ...
            <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data" class="space-y-6 h-adr">
                @csrf
                @method('PUT')
                ...
                <div class="flex flex-col items-center space-y-4">
                    <img id="currentProfileImg"
                        src="{{ $user->image ? asset($user->image) : asset('images/default-user.png') }}" alt="プロフィール画像"
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input id="name" name="name" type="text" value="{{ old('name', $user->name) }}"
                            class="mt-1 block w-full px-3 py-2 rounded border @error('name') border-red-500 @else border-gray-300 @enderror focus:outline-none focus:ring-2 focus:ring-red-400">
                        @error('name')
                            <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input id="postal_code" name="postal_code" type="text"
                            ...
                            class="p-postal-code mt-1 block w-full px-3 py-2 rounded border @error('postal_code') border-red-500 @else border-gray-300 @enderror focus:outline-none focus:ring-2 focus:ring-red-400">
                        @error('postal_code')
                            <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input id="address" name="address" type="text"
                            ...
                            class="p-region p-locality p-street-address p-extended-address mt-1 block w-full px-3 py-2 rounded border @error('address') border-red-500 @else border-gray-300 @enderror focus:outline-none focus:ring-2 focus:ring-red-400">
                        @error('address')
                            <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input id="building" name="building" type="text"
                            ...
                            class="mt-1 block w-full px-3 py-2 rounded border @error('building') border-red-500 @else border-gray-300 @enderror focus:outline-none focus:ring-2 focus:ring-red-400">
                        @error('building')
                            <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
